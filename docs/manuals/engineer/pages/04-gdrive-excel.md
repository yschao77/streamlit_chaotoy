...
`consumed=False` 只出現在狀態頁，不寫入主表。`preorder_orders` 已改為 consumed：預購追蹤「對訂單」子頁看板會讀最新 `Orders_DD-MM-YYYY-*.zip` 並解成 xlsx。本機上傳只預覽，禁止把 All Orders `files.create` 到 Drive。
...
| 預購追蹤.xlsx | `活動`、`廠商單歷史`（活動含 `貨號`、`廠商檔名`） |
| 入庫明細索引.xlsx | `明細`、`來源檔` |
