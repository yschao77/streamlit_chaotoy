...
# =========================================================================
# 🌐 1. Google Drive API 雲端連線與 5 大資料夾 ID 精準定義
# =========================================================================
...
def init_drive_service():
    """讀取設定在 Streamlit Secrets 的 textkey 金鑰字典並建立雲端連線"""
    try:
        ...
        st.error(f"❌ 無法從 Streamlit Secrets 中讀取 `textkey` 憑證。錯誤: {str(e)}")
        st.info("💡 請確保您在雲端後台的 Secrets 內已貼入完整的 Service Account JSON 資訊。")
        st.stop()
...
# =========================================================================
# 🔍 2. 雲端大數據自動檢測與讀寫核心工具函式
# =========================================================================
def find_gdrive_file(folder_id, file_name_keyword):
    """在指定的 Google Drive 資料夾中，模糊搜尋檔案並返回其 ID、修改時間與全名"""
    try:
...
def download_gdrive_file_to_bytes(file_id):
    """將雲端檔案直接下載至記憶體 (BytesIO) 中，免去硬碟讀寫權限"""
...
def upload_or_update_gdrive_file(folder_id, file_name, file_bytes, existing_file_id=None):
    """自動判定：若檔案存在則覆寫更新，不存在則在指定資料夾全新建立"""
...
ID_LOCAL_SHOPEE, TIME_SHOPEE, NAME_SHOPEE = find_gdrive_file(ID_SHOPEE_FOLDER, "蝦皮賣場商品列表")
ID_PRICE_SUMMARY, TIME_SUMMARY, NAME_SUMMARY = find_gdrive_file(ID_PRICE_SUMMARY_FOLDER, "商品蝦皮麗嬰價格統整表")

# =========================================================================
# ⚙️ 3. 全域雲端資料庫大數據預先連線載入
# =========================================================================
if ID_MASTER_FILE:
    try:
    ...
    st.error(f"❌ 於雲端 `麗嬰採購統整` 資料夾中找不到核心檔案 【麗嬰採購產品總表.xlsm】。")
    st.stop()
...
if "商品蝦皮麗嬰統整管理" in main_module:
    ...
    sub_page = st.sidebar.radio(
        "請選擇執行項目：",
        ["🔀 sitegiant 採購入庫單轉換", "📜 歷史入庫單管理與財務看板"],
        index=0
    )
...
# -------------------------------------------------------------------------
if sub_page == "📊 三表整合歷史":
    st.subheader("🔄 歷史三表整合比對紀錄追蹤")
    st.caption("系統會即時掃描雲端 `價格統整表/` 資料夾，方便您快速查看過往的比對清單並重新下載 Excel。")
    ...
    if not hist_pq_files:
        ...
        if selected_pq_file:
            try:
                ...
                st.markdown(f"📄 **目前調閱檔案**：`{selected_pq_file}` ｜ 📊 **資料總項數**：`{len(df_pq_view)} 筆`")
                ...
                st.error(f"❌ 讀取歷史備份檔案失敗: {str(e)}")

# -------------------------------------------------------------------------
# 子功能 2：🧠 PowerQuery 三表整合 (雲端相容版)
# -------------------------------------------------------------------------
elif sub_page == "🧠 PowerQuery 三表整合":
    st.subheader("🔍 雲端數據源新鮮度追蹤")
    st.info(f"☁️ 模式狀態: **Google Cloud API 直連流式傳輸模式**")    
    ...
    with c2:
        st.metric("🧡 蝦皮商品清單 (蝦皮商品列表)", "已對接 (Cloud)" if ID_LOCAL_SHOPEE else "❌ 雲端未偵測到")
    ...
    if st.button("🛠️ 啟動智慧勾稽整合與財務指標計算", type="primary", use_container_width=True):
        if not (ID_LOCAL_PROD and ID_LOCAL_SHOPEE and ID_MASTER_FILE):
            st.error("❌ 無法啟動智慧勾稽！請確認雲端硬碟各資料夾中是否齊備主核心檔案。")
        else:
            with st.spinner("正在進行大數據跨表比對與財務指標計算，請稍候..."):
                try:
                    ...
                    df_s = pd.read_excel(download_gdrive_file_to_bytes(ID_LOCAL_SHOPEE), sheet_name="蝦皮商品列表")
                    ...
                    backup_filename = f"整合單_{timestamp}.xlsx"
                    
                    out_buf = io.BytesIO()
                    df_final.to_excel(out_buf, index=False)
                    
                    # 雙軌上傳：1. 歷史紀錄存底 2. 覆寫統整表最新狀態
                    ...
                    st.success(f"🎉 智慧跨表勾稽計算完成！已成功備份至雲端歷史紀錄：{backup_filename}")
                except Exception as e:
                    st.error(f"❌ 錯誤: {str(e)}")
...
# -------------------------------------------------------------------------
    ...
    try:
        ...
        if search_term:
            ...
            st.caption("💡 提示：本表格已啟用 Streamlit 高效能滾動引擎，直接載入全量雲端數據。")
    except Exception as e:
...
# -------------------------------------------------------------------------
    ...
    if uploaded_files:
        if st.button("🚀 開始一鍵合併歸檔", type="primary"):
                ...
                try:
                    ...
                    for _, src_row in df_src.dropna(subset=['條碼']).iterrows():
                        ...
                        if barcode in master_dict:
                            ...
                            if name and str(master_row['名稱']).strip() != name:
                                need_insert = True
                            else:
            ...
            if save_to_master_xlsm({"麗嬰國際產品總表": df_total, "已匯入採購單": df_history, "metadata": df_meta}):
                st.session_state['merge_success_msg'] = f"🎉 成功完成雲端歸檔與寫入！順利處理 {success_count} 份檔案。 (跳過重複檔: {dup_count})"
                st.rerun()
    ...
    if not df_total.empty:
            ...
            if st.button("🧹 執行審核與刪除紀錄封存", type="primary"):
                ...
                if len(uids_to_delete) > 0:
                    ...
                    if save_to_master_xlsm({"麗嬰國際產品總表": df_remaining, "刪除紀錄": df_delete_log}):
                        st.session_state['merge_success_msg'] = "🧹 雲端歸檔移轉與除錯清洗順利執行完畢！"
                        st.rerun()
        else:
        ...
        with pd.ExcelWriter(towrite, engine='openpyxl') as writer:
            df_export = df_total.drop(columns=['move'], errors='ignore').copy()
            df_export.to_excel(writer, index=False, sheet_name="麗嬰國際產品總表")
        st.download_button(label="📥 匯出標準麗嬰產品總表", data=towrite.getvalue(), file_name=f"麗嬰國際產品總表_{datetime.date.today().strftime('%Y%m%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

# -------------------------------------------------------------------------
# 子功能 5：📈 蝦皮商品清單轉換
# -------------------------------------------------------------------------
elif sub_page == "📈 蝦皮商品清單轉換":
    st.subheader("🛍️ 蝦皮賣場商品列表欄位優化對齊")
    uploaded_shopee = st.file_uploader("📥 上傳新的蝦皮原始報表 (.xlsx) 進行格式校正：", type=["xlsx"], key="main_shopee_upload")
    
    if uploaded_shopee:
        if st.button("🪄 執行蝦皮大清洗與結構校正", type="primary", use_container_width=True):
            try:
                ...
                # 自動更新至雲端專屬資料夾中
                out_buf = io.BytesIO()
                df_final_clean.to_excel(out_buf, index=False)
                upload_or_update_gdrive_file(ID_SHOPEE_FOLDER, "蝦皮賣場商品列表.xlsm", out_buf.getvalue(), existing_file_id=ID_LOCAL_SHOPEE)
                
                st.session_state['shopee_clean'] = df_final_clean
                st.success("✨ 蝦皮欄位標準化與去重清洗校正成功，並已同步寫入雲端 Master 檔！")
            except Exception as e:
    ...
    if 'shopee_clean' in st.session_state:
        ...
        st.download_button(label="📥 下載標準化清洗後蝦皮報表 (.xlsx)", data=towrite_shopee.getvalue(), file_name=f"蝦皮清洗完成對齊表_{datetime.date.today().strftime('%Y%m%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

# -------------------------------------------------------------------------
# 子功能 6：🔀 sitegiant 採購入庫單轉換 (雲端相容版)
# -------------------------------------------------------------------------
elif sub_page == "🔀 sitegiant 採購入庫單轉換":
    st.subheader("🛍️ SiteGiant 採購入庫單智慧轉換中心")
    
    if ID_PRICE_SUMMARY:
        st.success(f"🟢 雲端價格統整表主核心已成功建立串聯")
    else:
        st.error(f"❌ 請先前往執行『🧠 PowerQuery 三表整合』以產生雲端價格統整主核心。")
    ...
    bulk_paste_area = st.text_area("請直接在此處貼上條碼明細 (Tab鍵或空白鍵隔開)：", height=120, placeholder="條碼 數量")
    ...
    if st.button("✨ 執行轉換與精準對帳比對", type="primary", use_container_width=True) and ID_PRICE_SUMMARY:
        if not order_no.strip() or input_df.empty: 
            st.error("❌ 轉換失敗！請填入銷貨單號與有效明細。")
        else:
            try:
                df_ref = pd.read_excel(download_gdrive_file_to_bytes(ID_PRICE_SUMMARY))
                ...
                for row in input_df.itertuples(index=False):
                    ...
                    if not match.empty:
                        ...
                        tax = match_row.get('麗嬰稅款', 0)
                        cost = match_row.get('麗嬰未稅價', 0)
                        
                        result_rows.append({
                            "收貨日": str(recv_date), "國際條碼": barcode_input,
                            "庫存SKU": "⚠️ 提示：須新增iSKU" if pd.isna(sku) else sku, "庫存貨品名稱": prod_name, 
                            "成本": "⚠️ 提示：手動輸入" if pd.isna(cost) else cost, "稅款": "⚠️ 提示：手動輸入" if pd.isna(tax) else tax, "數量": qty,
                            "分類定義": category, "產品關鍵字": keywords
                        })
                    else:
                        result_rows.append({
                            "收貨日": str(recv_date), "國際條碼": barcode_input, "庫存SKU": "⚠️ 提示：須新增iSKU", 
                            "庫存貨品名稱": "請至麗嬰總表補入此新商品", "成本": "⚠️ 提示：手動輸入", "稅款": "⚠️ 提示：手動輸入", "數量": qty, "分類定義": "", "產品關鍵字": ""
                        })
    ...
    if 'inward_result_df' in st.session_state:
        ...
        st.success(f"💾 檔案已同步封存至雲端歷史目錄。")
        st.download_button(label=f"📥 下載入庫對帳單 ({final_filename})", data=towrite_inward.getvalue(), file_name=final_filename, mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", type="primary")

# -------------------------------------------------------------------------
# 子功能 7：📜 歷史入庫單管理與財務看板 (雲端相容版)
# -------------------------------------------------------------------------
elif sub_page == "📜 歷史入庫單管理與財務看板":
    st.subheader("📊 歷史入庫單與重要財務加總指標檢視")
    st.caption("系統會自動追蹤雲端 `歷史入庫單/` 資料夾下的文件，為您呈現進貨總成本與營業稅額加總統計。")
    
    hist_files = list_gdrive_files(ID_HISTORY_INWARD_FOLDER)
        
    if not hist_files:
        st.warning(f"💡 目前雲端內無歷史單據，請先至『🔀 sitegiant 採購入庫單轉換』建立單據報告。")
    else:
        ...
        if selected_hist_file:
            try:
                ...
                st.markdown(f"📄 **當前檔案**：`{selected_hist_file}` ｜ 📊 **單據品項數**：`{len(df_hist_view)} 筆`")
                st.dataframe(df_hist_view, use_container_width=True)
                
                h_cost = 0.0; h_tax = 0.0
                ...
                st.markdown("#### 📊 本張單據入庫財務統計加總看板")
                c_tot1, c_tot2 = st.columns(2)
                with c_tot1: st.metric(label="💰 歷史成本未稅總金額 (成本 * 數量)", value=f"$ {h_cost:,.2f} 元")
                with c_tot2: st.metric(label="🧾 歷史營業稅總金額 (稅款 * 數量)", value=f"$ {h_tax:,.2f} 元")
