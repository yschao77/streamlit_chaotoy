...
# =========================================================================
# 將您指定的核心網路空間 Google Cloud 雲端硬碟物件 ID 全面導入
...
def init_drive_service():
    ...
    try:
        ...
        st.info("💡 請確認您的 Secrets 設定格式是否正確（必須包含 textkey 區塊，並具備完整 GCP Service Account 欄位）。")
        st.stop()
...
def list_gdrive_files(folder_id):
    """列出雲端資料夾內的所有 Excel 檔案 (取代 os.listdir)"""
    try:
        ...
        files = results.get('files', [])
        # 依名稱倒序排列，對應本地端的 sort(reverse=True)
...
def upload_or_update_gdrive_file(folder_id, file_name, file_bytes, existing_file_id=None):
    """上傳新檔案到指定的 Google Drive 資料夾或直接在原 ID 上更新/覆寫現有檔案"""
...
# =========================================================================
# ⚙️ 3. 核心資料庫讀寫雲端常式 (取代原本本地 wb.save)
# =========================================================================
...
def save_to_shopee_master_xlsm(sheets_dict):
    global ID_SHOPEE_MASTER # 修正：宣告移到最頂端，避免語法編譯衝突
    try:
        if ID_SHOPEE_MASTER:
...
if ID_MASTER_FILE:
    try:
        ...
        st.error(f"🔴 讀取雲端主資料庫失敗，請確認該硬碟空間內是否有『麗嬰採購產品總表.xlsm』。錯誤: {str(e)}")
        st.stop()
else:
    st.error(f"❌ 雲端資料庫內找不到核心總表檔案 【麗嬰採購產品總表.xlsm】，請確認共用權限設定。")
    st.stop()
...
if sub_page == "📊 PowerQuery 三表整合歷史紀錄":
    ...
    st.caption("系統已改為即時掃描雲端 `價格統整表/` 資料夾，方便您快速查看過往比對清單並重新下載 Excel。")
...
# -------------------------------------------------------------------------
    ...
    with c1:
        st.metric("📦 商品列表 (商品iSKU清單)", "已對接 (Cloud)" if ID_LOCAL_PROD else "❌ 未偵測到")
        st.caption(f"📅 最後修改時間: \n`{format_gdrive_time(TIME_PROD)}`")
    with c2:
        st.metric("🧡 蝦皮資料庫主表 (蝦皮商品列表)", "已對接 (Cloud)" if ID_SHOPEE_MASTER else "❌ 未偵測到")
        st.caption(f"📅 最後修改時間: \n`{format_gdrive_time(TIME_SHOPEE)}`")
    with c3:
        st.metric("🧸 麗嬰產品總表", "已對接 (Cloud)" if ID_MASTER_FILE else "❌ 未偵測到")
        st.caption(f"📅 最後修改時間: \n`{format_gdrive_time(TIME_MASTER)}`")

    st.markdown(f"**📊 價格統整目標輸出雲端路徑狀態：** `已配置專屬雲端對接點`")
    st.write("---")
...
# -------------------------------------------------------------------------
    ...
    st.info(f"💾 目前雲端主資料庫修改時間：{format_gdrive_time(TIME_MASTER)}")
    ...
    if not df_total.empty:
        ...
        if not df_anomaly.empty:
            ...
            edited_anomaly_df = st.data_editor(
                ...
                disabled=[col for col in df_anomaly.columns if col not in ['move', '備註']], 
                key="anomaly_editor"
            )
            ...
            if st.button("🧹 執行審核與資料儲存", type="primary"):
                for index, edited_row in edited_anomaly_df.iterrows():
                    ...
                    new_note = str(edited_row['備註']).strip()
                    clean_note = new_note.replace("🔴 ", "").replace("🟢 ", "")
                    df_total.loc[df_total['UID'] == target_uid, '備註'] = clean_note
                ...
                if len(uids_to_delete) > 0:
                    ...
                    df_remaining_only = df_total.drop(columns=['move'], errors='ignore')
                    df_remaining_only = run_cross_matching(df_remaining_only)
...
# -------------------------------------------------------------------------
    ...
    st.caption("本系統會將轉換成果同步對接雲端備份，並全面更新至雲端 `蝦皮商品清單/蝦皮賣場商品列表.xlsm` 的 `[蝦皮商品列表]` 內。")
    ...
    if uploaded_shopee:
        ...
        if shopee_md5 in df_shopee_history['md5'].astype(str).values:
            st.error(f"⚠️ 拒絕重複格式校正！偵測到本次上傳的檔案 `{uploaded_shopee.name}` 的內容與雲端歷史紀錄完全一致（MD5 相同），系統已自動封鎖。")
        else:
            if st.button("🪄 執行蝦皮iSKU結構校正", type="primary", use_container_width=True):
                try:
                    ...
                    new_hist_log = pd.DataFrame([{
                        "檔案名稱": uploaded_shopee.name,
                        "md5": shopee_md5,
                        "匯入時間": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    }])
                    ...
                    if save_to_shopee_master_xlsm({"蝦皮商品列表": df_final_clean, "匯入檔案": df_shopee_history}):
                        ...
                        st.success(f"🎉 格式標準化校正成功！已完成任務：\n1. 雲端備份檔案已存底：`{backup_sp_filename}`\n2. 雲端 `蝦皮賣場商品列表.xlsm` 已同步完整更新！")
                except Exception as e:
        ...
        if not order_no.strip() or input_df.empty: 
            ...
            try:
                if ID_PRICE_SUMMARY:
                    ...
                    st.error("❌ 雲端找不到『商品蝦皮麗嬰價格統整表』，請先執行三表整合。")
            except Exception as e: 
                st.error(f"❌ 錯誤: {str(e)}")
    ...
    if 'inward_result_df' in st.session_state:
        ...
        for col in ["成本", "稅款"]:
            if col in df_download.columns:
                df_download[col] = pd.to_numeric(df_download[col], errors='coerce').fillna(0.0)
        ...
        edited_inward_df = st.data_editor(
            ...
            column_config={
                "成本": st.column_config.NumberColumn("成本", help="請輸入未稅單件成本", min_value=0.0, format="$ %.2f"),
                "稅款": st.column_config.NumberColumn("稅款", help="請輸入單件營業稅額", min_value=0.0, format="$ %.2f"),
            },
...
# -------------------------------------------------------------------------
    ...
    st.caption("系統已改為全自動追蹤雲端 `歷史入庫單/` 空間內的文件，為您呈現進貨總成本與營業稅額加總統計。")
    
    hist_files = list_gdrive_files(ID_HISTORY_INWARD_FOLDER)
        
    if not hist_files:
        st.warning(f"💡 目前雲端 `歷史入庫單/` 空間內無歷史單據，請先至『🔀 sitegiant 採購入庫單轉換』建立。")
    else:
...
# -------------------------------------------------------------------------
    ...
    st.caption("系統已改為全自動追蹤雲端 `商品列表/` 空間下的文件，為您呈現目前最新商品清單iSKU及分類。")

    hist_files = list_gdrive_files(ID_PROD_FOLDER)
        
    if not hist_files:
        st.warning(f"💡 目前雲端 `商品列表/` 空間內無歷史單據。")
    else:
        file_options = {f['name']: f['id'] for f in hist_files}
